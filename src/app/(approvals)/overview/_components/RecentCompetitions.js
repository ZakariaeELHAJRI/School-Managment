...
const RecentCompetitions = ({ title }) => {
	...
	const fetchAllCompetitions = async () => {
		try {
			...
			console.log('All competitions', response.data);
		} catch (error) {
		...
		<Card className="h-100">
			...
			<Card.Body>
				<ListGroup variant="flush">
					{allcompetitions.slice(0, 4).map((item, index) => (
						<ListGroup.Item
							...
							<Row>
								...
								<Col className='d-flex flex-column m-auto'>
									...
									<div className="d-flex align-items-center">
										...
										<span className="fs-6">{truncateText(item.description, 15)}</span>
									</div>
								</Col>
