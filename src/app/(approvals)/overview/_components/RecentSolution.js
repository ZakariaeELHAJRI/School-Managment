	  ...
	  <Card className="h-100">
		...
		<Card.Body>
		  <ListGroup variant="flush">
			{allsolutions.slice(0, 4).map((item, index) => (
			  <ListGroup.Item
			  ...
			  >
				<Row>
				  ...
				  <Col className='d-flex flex-column m-auto'>
					<Link href="#">
					  <h5 className="text-primary-hover">{truncateText(item.solution, 15)}</h5>
					</Link>
					<div className="d-flex align-items-center">
					  <span className="fs-6">{(getCourseNameById(item.course_id), 20)}</span>
					</div>
					<div className="d-flex align-items-center">
