...
const truncateText = (text, maxLength) => {
  ...
  const RecentExercice = ({ title }) => {
	const storageUrl = process.env.NEXT_PUBLIC_STORAGE;
	const [allexercices, setAllExercice] = useState([]);
	...
	const fetchAllExercice = async () => {
	  try {
		const response = await axiosInstance.get('/exercice');
		setAllExercice(response.data);
		console.log('All exercices', response.data);
	  } catch (error) {
	  ...
	  <Card className="h-100">
		...
		<Card.Body>
		  <ListGroup variant="flush">
			{allexercices.slice(0, 4).map((item, index) => (
			  <ListGroup.Item
